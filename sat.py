...
# Resolver el problema y encontrar todas las soluciones
solutions = []
with Solver(bootstrap_with=cnf) as solver:
    while solver.solve():
        solution = solver.get_model()
        # Convertir la solución a 0 y 1
        binary_solution = [(1 if i > 0 else 0) for i in solution[:20]]
        solutions.append(binary_solution)
        
        # Añadir una cláusula para bloquear la solución actual y encontrar otra
        solver.add_clause([-lit for lit in solution])

# Mostrar todas las soluciones en 0 y 1
